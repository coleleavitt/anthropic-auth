...
- `packages/opencode/src/claustrum-enrollment-registry.ts`: Process-shared `anthropic-auth-opencode` enrollment lifecycle at an owner-only token path; scoped serving uses that token for each outbound attempt
