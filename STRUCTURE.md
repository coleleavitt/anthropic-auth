...
**New slash command:** Register the command name in `packages/core/src/constants.ts` (or a dedicated module), implement execution logic in core (shared) or per-package (if platform-specific), then wire it at all four sites: the `CommandModalName` union in `packages/opencode/src/rpc/protocol.ts` (the closed type behind the modal list), plus three in `packages/opencode/src/index.ts` — the `config` hook's command registration keyed by the command's `COMMAND_NAME` constant with `template` and `description`, the `modalCommands` list inside `command.execute.before`, and `buildDialogPayload`. Missing any site fails differently. Unit tests cover the registration surfaces (`index.test.ts` asserts every modal command registers through the `config` hook), but only an interactive palette check verifies the running host actually surfaces the command. For Pi, add the command in `packages/pi/src/commands.ts`. If the command owns a background scheduler, keep its manager in core and adopt one process-wide manager through a keyed registry in the host package; persist user opt-in separately from runtime counters when the command spans processes, as `/claude-prime` does.
