...
2. **Provider registration** — Provider defines localhost-first OAuth login plus manual fallback and refresh functions. Pi's fixed three-field credential record remains host-native, while refresh expiry and rotations are mirrored through the canonical shared store — `packages/pi/src/index.ts`
