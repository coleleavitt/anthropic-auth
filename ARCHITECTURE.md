...
- Pattern: The top-level `output_config.effort` remains anchored to the earliest retained effort. Later changes become empty `role: "system"` messages immediately before their user/tool-result boundary. OAuth Fable 5.1 requests always include `mid-conversation-output-config-2026-07-01`; other models and API-key routes do not. Before OpenCode lowers host records into Anthropic messages, it adds nonce-bound HMAC-authenticated transition markers to lowerable user boundaries and a signed plan marker to the current user turn. The fetch transform authenticates and strips those markers, inserts the empty system messages at the surviving structural boundaries, and fails locally on missing, duplicate, or mismatched marker state before cache shaping, billing-suffix derivation, dumping, signing, or transport. This avoids assistant-record count assumptions that fail when OpenCode collapses tool-heavy or compacted histories. Pi rebuilds transitions directly from active compaction-aware session entries on each turn.
