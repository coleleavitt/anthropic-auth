...
- Contains: OAuth authorization/refresh/revocation (`auth.ts`) and localhost completion (`oauth-loopback.ts`), native Claude discovery/import plus explicit secure storage (`native-claude-credentials.ts`, `secure-secret-store.ts`), shared account storage (`shared-account-store.ts`, `shared-account-adapter.ts`, `accounts.ts`), Workload Identity Federation (`wif.ts`), persistent installation identity (`device-identity.ts`, `claude-code.ts`), trusted-device/attestation/Cowork contracts (`trusted-device.ts`, `attestation.ts`, `cowork-binding.ts`), 2.1.260 billing-header compatibility plus legacy CCH diagnostics (`cch.ts`), profile fetching (`oauth-profile.ts`), PKCE generation (`pkce.ts`), token fingerprinting (`token-fingerprint.ts`), quota management (`quota-manager.ts`, `quotas.ts`, `quota-headers.ts`), quota priming (`prime.ts`), explicit lane starts (`start.ts`), cache control (`cache1h.ts`, `cachekeep.ts`, `cachekeep-registry.ts`), relay/dump/routing/killswitch/fast-mode/model/logging/provider and account-command logic
- Depends on: `xxhash-wasm` (for legacy CCH/cache diagnostics), Node.js built-ins (`buffer`, `crypto`, `fs`, `os`, `path`)
...
2. **Auth loader** — The plugin reconciles the canonical shared credential, OpenCode auth, inherited environment auth, then WIF. Claude subscription OAuth starts locked background refresh; WIF gets a dedicated standard-bearer fetch path with no Claude Code identity/body rewrite — `packages/opencode/src/index.ts`, `packages/opencode/src/shared-auth.ts`
...
1. **Extension load** — Pi loads `@cortexkit/pi-anthropic-auth` package, which calls `registerCommands()`, seeds Pi's own credential file through `adoptSharedCredentialIntoHostAuth()`, then calls `pi.registerProvider("anthropic", ...)`. Seeding is required because Pi gates every run on `ModelRegistry.hasConfiguredAuth`, which reads only Pi's `AuthStorage` (`auth.json`, environment keys, runtime overrides, provider-config `apiKey`), while the request path reads the canonical shared store directly; without a seed an empty `auth.json` refuses an otherwise fully authenticated machine with "No API key found for anthropic" before `streamSimple` is ever called. The seed prefers a live account, falls back to an expired-but-refreshable one (Pi's stored-credential gate ignores expiry and then routes through `refreshToken`), never overwrites an existing host entry, and never blocks provider registration — `packages/pi/src/index.ts`, `packages/pi/src/adopt-host-credential.ts`
2. **Provider registration** — Provider defines localhost-first OAuth login plus manual fallback and refresh functions (delegating to core's `authorize`/`exchange`/`refreshClaudeOAuthToken`) and exposes the CortexKit model catalog including Opus 5. Pi's fixed three-field credential record remains host-native, while refresh expiry and rotations are mirrored through the canonical shared store — `packages/pi/src/index.ts`
...
4. **Stream implementation** — `streamCortexKitAnthropic()` in `packages/pi/src/stream.ts` sends via relay or direct, handles ordered or persistent sticky-balanced routing (including model-scoped OAuth routing and the confirmed-general-exhaustion gate for API-key routes), and cache keepalive. Eligible OAuth Fable 5/Opus 5 requests opt into Anthropic's default server-side safety fallback. Pi stores unsupported `fallback` blocks as signed hidden thinking markers and restores them on the next request. An unrecovered refusal preserves both host raw-stop field spellings and emits a structured `provider_stream_failure` diagnostic so current Prime hosts classify it as permanent after at most their first compatibility retry instead of exhausting the retry budget on the billed turn. If a fallback response completed a tool call before refusing, Pi preserves that call and finishes as `toolUse` so the existing result can continue without replay. Custom API-key routes strip internal markers and never send the server-fallback beta or request field. WebSocket relay requests propagate the caller's abort signal through connect, queue, response, and stream phases; cancellation closes the socket after clearing pending state because binary relay chunks carry no request ID. — `packages/pi/src/stream.ts`, `packages/core/src/relay.ts`
...
- Purpose: Project-neutral source of truth for Anthropic OAuth and first-party API-key credentials, compatible with the Rust `anthropic` crate
- Location: `packages/core/src/shared-account-store.ts`, with sidecar conversion in `shared-account-adapter.ts` and OpenCode resolution in `packages/opencode/src/shared-auth.ts`
- Pattern: `~/.anthropic-accounts/accounts.json`, overridden by `ANTHROPIC_ACCOUNTS_FILE` or `ANTHROPIC_ACCOUNTS_DIR`. Ordered accounts plus a pinned `current` ID; tagged `oauth` and first-party `api_key` credentials; canonical → OpenCode → inherited environment precedence. Custom proxy routes remain sidecar-only because the shared schema cannot preserve their endpoint/header provenance safely. Atomic fsync+rename writes, cross-process lock, stale-temp sweep, direct store-file symlink refusal, accidental-empty guard with an explicit final-removal escape hatch, refresh-token compare-and-swap, optional refresh expiry, and Unix `0700`/`0600` permissions. Existing OpenCode auth and valid sidecar fallbacks are adopted when needed; newer host/fallback token rotations synchronize back into the canonical store.

**AccountStorage (sidecar file):**
- Purpose: Persisted plugin configuration and runtime routing state — fallback labels/routes, quotas, refresh backoff, killswitch settings, cache/relay config
- Location: `packages/core/src/accounts.ts`
- Pattern: JSON file on disk (`~/.config/opencode/anthropic-auth.json` for OpenCode, `~/.pi/agent/anthropic-auth.json` for Pi). Runtime quota/backoff state and a compatibility credential mirror live in `anthropic-auth-state.json`; canonical credentials live in the shared account store; sticky routing affinity lives in `anthropic-auth-routing-state.json`. Atomic writes via temp + rename. Serializes all account-store and state writes via an in-process save mutex and acquires a cross-process write lock on the configuration file during save. Merges existing accounts on disk with incoming accounts on save to prevent fallback account loss under concurrent execution. Refresh file lock acquisition tolerates `ENOENT` and `EINVAL` lost-parent races when creating eviction marker files under high contention. Main refresh errors are refresh-token-hash-bound: rotating the refresh token clears a permanent old-token `invalid_grant`, while active transient/shared backoffs remain. Preserves empty scoped quota arrays `[]` on load/merge to distinguish explicit zero-carve-out from missing quota metadata.
...
- Responsibilities: Localhost-first OAuth and API-route login, explicit native-Claude import, confirmed remote OAuth revocation, account listing, and relay setup (Cloudflare Worker provisioning)
...
**Security:** Canonical OAuth/API credentials live in private shared storage; custom-route compatibility secrets remain in the sidecar state file. Native keychain reads are bounded and shell-free, plaintext fallbacks and auxiliary trusted/Cowork secrets use `0700`/`0600` atomic stores, secret-bearing debug/errors are redacted, sticky routing stores only SHA-256 session hashes and quota metadata, and token/config/routing writes use owner-checked stale-recovering locks; no secrets are stored in git
