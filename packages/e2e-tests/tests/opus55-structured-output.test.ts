...
test('Opus 5.5 structured output keeps the tool available and fails closed if it is not called', async () => {
  ...
  harness.script([{ type: 'tool_use', name: 'mcp_StructuredOutput', input: { answer: 'ok' } }])
  ...
  const first = harness.anthropic.requests().find((request) =>
    request.body.model === 'claude-opus-5-5' &&
    Array.isArray(request.body.tools) &&
    request.body.tools.some((tool: { name?: string }) => tool.name === 'mcp_StructuredOutput'),
  )
  expect(first).toBeDefined()
  expect(first?.body.tool_choice).toBeUndefined()
  expect(first?.body.thinking).toMatchObject({ type: 'adaptive' })

  harness.script([{ type: 'text', text: 'A plain-text answer, not structured output.' }])
  ...
  expect(JSON.stringify(failureInfo?.error)).toContain('Model did not produce structured output')
}, 90_000)
