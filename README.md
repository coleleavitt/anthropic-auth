...
| Workload Identity Federation | Automatic when official WIF env wins precedence | Core provider is available; Pi's host auth schema cannot represent an async refreshless provider |
| Shared Anthropic credentials | `~/.anthropic-accounts/accounts.json` (OAuth and first-party API key) | Pi keeps its native three-field auth record and mirrors OAuth/refresh-expiry rotations into the shared store |
...
- **Complete Claude auth lifecycle**: automatic localhost OAuth callbacks with manual fallback, persisted access/refresh expiries, compare-and-swap token rotation, explicit remote revocation, and opt-in import from native Claude secure storage or `~/.claude/.credentials.json`.
- **Workload Identity Federation**: first-party organization/workspace WIF with optional service-account targeting, official environment precedence, rotating projected identity-token files, single-flight refresh, and a standard bearer route that never receives Claude subscription identity/body rewrites.
- **Device compatibility**: a persistent installation-wide 32-byte device ID, auxiliary private trusted-device storage/enrollment, Remote Control attestation filtering, and Cowork P-256 registration/session binding helpers.
...
The Pi package registers under Pi's built-in `anthropic` provider ID and stores primary OAuth credentials through Pi's normal credential flow. Because Pi's host schema has only access/refresh/access-expiry fields, CortexKit mirrors refresh-token expiry and rotations into the canonical shared account store. Package settings and runtime state remain in the Pi sidecar.
...
Path resolution is: `ANTHROPIC_ACCOUNTS_FILE`, then `ANTHROPIC_ACCOUNTS_DIR/accounts.json`, then the default above. The JSON schema supports ordered accounts, a pinned `current` account, OAuth credentials (including optional `refresh_expires_at` epoch milliseconds), and `api_key` credentials. OAuth requests use `Authorization: Bearer` plus `anthropic-beta: oauth-2025-04-20`; first-party API keys stay on OpenCode's built-in Anthropic path and use `x-api-key`. Installation identity lives separately in `~/.anthropic-accounts/device.json`; trusted-device tokens and Cowork private keys never enter `accounts.json`.
...
Known Rust-schema-compatible legacy shared-store locations under `~/.config/jfc`, `~/.grok`, and `~/.config/opencode` are read when the canonical file does not exist; older incompatible application-specific schemas are skipped rather than treated as canonical credentials. Store writes refuse directly symlinked files, use user-only permissions on Unix, and reject accidental empty writes; the explicit final-account removal/revocation path can persist a valid empty version-1 store.
...
Prefer npm? Use `npx -y @cortexkit/opencode-anthropic-auth ...` with the same subcommands. Native import requires typing `IMPORT`; remote revocation requires typing `revoke`. The equivalent slash commands require `--confirm`.
...
| `ANTHROPIC_API_KEY` | Non-persisted first-party API-key fallback used after `ANTHROPIC_OAUTH_TOKEN`. Its presence (even empty) shadows WIF, matching the official SDK. |
| `ANTHROPIC_FEDERATION_RULE_ID` | Enable Workload Identity Federation when paired with organization and identity-token settings. |
| `ANTHROPIC_ORGANIZATION_ID` | Anthropic organization for WIF exchange. |
| `ANTHROPIC_SERVICE_ACCOUNT_ID` | Optional service account receiving the federated token. |
| `ANTHROPIC_WORKSPACE_ID` | Optional workspace selector for multi-workspace federation rules. |
| `ANTHROPIC_IDENTITY_TOKEN_FILE` / `ANTHROPIC_IDENTITY_TOKEN` | Rotating projected OIDC assertion file (preferred) or inline assertion. |
| `CLAUDE_TRUSTED_DEVICE_TOKEN` | Native trusted-device override for explicit Remote Control/Cowork requests only. Never attached to Messages requests. |
